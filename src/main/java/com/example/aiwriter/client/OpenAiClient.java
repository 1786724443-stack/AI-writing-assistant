...
public class OpenAiClient implements AiClient {
    ...
    public OpenAiClient(RestTemplate restTemplate, AiProperties properties) {
    ...
    public AiResponse generate(String prompt) {
        ...
        headers.set("Authorization", "Bearer " + properties.getOpenai().getApiKey());
        ...
        try {
            ...
            if (response.getStatusCode().is2xxSuccessful() && response.getBody() != null) {
                ...
                if (body.getChoices() != null && !body.getChoices().isEmpty()) {
                    ...
                    log.info("OpenAI request successful, tokens used: {}", tokenUsage);
                    return AiResponse.success(content, tokenUsage);
                }
            }
            log.warn("OpenAI response empty or invalid: {}", response.getStatusCode());
            return AiResponse.failure("Empty or invalid response from OpenAI");
